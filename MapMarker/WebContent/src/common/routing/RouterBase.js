...
function(declare, hash, topic) {
    ...
    function fireRoute(params, currentPath, newPath, segue) {
        ...
        if (params instanceof Array) {
            ...
            for (var key in params) { //jshint forin: false
    ...
    var RouterBase = declare(null, {
        ...
        registerSecured: function( /*String|RegExp*/ route, /*Function*/ callback) {
            // summary:
            //		Registers a route that requires authenticated user
            // description:
            //		Much like the `register` method, `registerSecured` allows
            //		us to register route that requires user to be authenticated
            //		if not authenticated, routes to login first

            return this._registerRoute(route, callback, true, true);
        },

        register404: function(callback) {
            this["404Callback"] = callback;
        },

        go: function(path, replace, segue) {
            // summary:
            ...
            path = trim(path);
            applyChange = this._handlePathChange(path, segue);
        ...
        startup: function(defaultPath, /*String*/ loginPath, /*Function*/ isAuthenticated) {
            // summary:
            //		This method must be called to activate the router. Until
            //		startup is called, no hash changes will trigger route
            //		callbacks.
            //		loginPath and isAuthenticated are option and 
            //		provided for authenticated routes
        ...
        _handlePathChange: function(newPath, segue) {
            ...
            if (app.formDirty) {
                app.growler.growl({
                    message: "This form has not been saved. Click 'Back' again to discard this form, or click 'Submit' to save your changes.",
                    level: "warning",
                    timeout: 0
                });
                ...
                return false;
            } else if (app.formDirty === false) {
                app.growler.clear();
                app.formDirty = undefined;
            }
            ...
            for (i = 0, li = routes.length; i < li; ++i) {
                ...
                if (result) {
                    ...
                    if (routeObj.secured && !this._isAuthenticated()) {

                        allowChange = false;

                        this.go(this._loginPath, false, {
                            redirectURL: newPath,
                            segue: routeObj.segue
                        });

                    } else {
                        allowChange = routeObj.fire(params, currentPath, newPath, segue);
                    }
                }
            }

            if (allowChange && this.loading && this["404Callback"]) {
                this["404Callback"]({
                    oldPath: currentPath,
                    newPath: newPath
                });
        ...
        _registerRoute: function( /*String|RegExp*/ route, /*Function*/ callback, /*Boolean?*/ isBefore, /*Boolean?*/ isSecured) {
