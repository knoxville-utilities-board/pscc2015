...
@Path("type")
public class TypeRSResource implements BeanFactoryAware{
	
    private TypeRSDao typeRSDao;
    ...
    public TypeRSResource() {
        if (beanFactory != null) {
            setTypeRSDao((TypeRSDao) beanFactory.getBean("typeRSDao"));
        }
    }
    ...
    public Response getList(@HeaderParam(value = "Range") String rangeHeader,
        ...
        ModelList types = typeRSDao.getTypes(page, filterList, sortList);

        return Response.ok(new KUBDataObject(types.getList()))
                .header(Range.CONTENT_RANGE_HEADER, types.getRange().toContentRange()).build();
    }
    ...
    public Response getById(@PathParam("id") String id) throws KUBGenericException {
        Type type = typeRSDao.getType(Integer.parseInt(id));

        if (type == null) {
            return Response.status(Response.Status.NOT_FOUND).build();
        }

        return Response.ok(new KUBDataObject(type)).build();
    }
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public Response create(Type type) throws KUBGenericException {
        type = typeRSDao.createType(type);
        return Response.ok(new KUBDataObject(type)).build();
    }
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    public Response update(Type type) throws KUBGenericException {
        type = typeRSDao.updateType(type);
        return Response.ok(new KUBDataObject(type)).build();
    }
    ...
    public Response remove(@PathParam("id") String id) throws KUBGenericException {
        typeRSDao.deleteType(Integer.parseInt(id));
    ...
    public void setBeanFactory(BeanFactory beanFactory) throws BeansException {
        ...
        TypeRSResource.beanFactory = beanFactory;
    }

    public TypeRSDao getTypeRSDao() {
        return typeRSDao;
    }

    public void setTypeRSDao(TypeRSDao typeRSDao) {
        this.typeRSDao = typeRSDao;
    }
