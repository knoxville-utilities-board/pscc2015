...
@Path("severity")
public class SeverityRSResource implements BeanFactoryAware{
	
    private SeverityRSDao severityRSDao;
    ...
    public SeverityRSResource() {
        if (beanFactory != null) {
            setSeverityRSDao((SeverityRSDao) beanFactory.getBean("severityRSDao"));
        }
    }
    ...
    public Response getList(@HeaderParam(value = "Range") String rangeHeader,
        ...
        ModelList severities = severityRSDao.getSeverities(page, filterList, sortList);

        return Response.ok(new KUBDataObject(severities.getList()))
                .header(Range.CONTENT_RANGE_HEADER, severities.getRange().toContentRange()).build();
    }
    ...
    public Response getById(@PathParam("id") String id) throws KUBGenericException {
        Severity severity = severityRSDao.getSeverity(Integer.parseInt(id));

        if (severity == null) {
            return Response.status(Response.Status.NOT_FOUND).build();
        }

        return Response.ok(new KUBDataObject(severity)).build();
    }
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public Response create(Severity severity) throws KUBGenericException {
        severity = severityRSDao.createSeverity(severity);
        return Response.ok(new KUBDataObject(severity)).build();
    }
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    public Response update(Severity severity) throws KUBGenericException {
        severity = severityRSDao.updateSeverity(severity);
        return Response.ok(new KUBDataObject(severity)).build();
    }
    ...
    public Response remove(@PathParam("id") String id) throws KUBGenericException {
        severityRSDao.deleteSeverity(Integer.parseInt(id));
    ...
    public void setBeanFactory(BeanFactory beanFactory) throws BeansException {
        ...
        SeverityRSResource.beanFactory = beanFactory;
    }

    public SeverityRSDao getSeverityRSDao() {
        return severityRSDao;
    }

    public void setSeverityRSDao(SeverityRSDao severityRSDao) {
        this.severityRSDao = severityRSDao;
    }
